from nicegui import app, ui, Client, events
from functions import plot_csv

app.add_static_files('/static', 'static')


class ModalDialog:
    """Create a dialog that can be opened to display some HTML content."""

    def __init__(self, html_content) -> None:
        with ui.dialog().props('maximized') as self.dialog:
            with ui.row():
                with ui.row().classes('px-4 pt-4'):  # add a row for close button
                    with ui.button(on_click=self.close).classes(
                            'justify-self-end'):
                        ui.icon('close')  # close button
                ui.html(html_content).classes("w-full h-full")

    def open(self) -> None:
        """Open the dialog."""
        self.dialog.open()

    def _on_key(self, event_args: events.KeyEventArguments) -> None:
        if not event_args.action.keydown:
            return
        if event_args.key.escape:
            self.dialog.close()

    def close(self) -> None:
        """Close the dialog."""
        self.dialog.close()

    # def _on_key(self, event_args: events.KeyEventArguments) -> None:
    #     """Close the dialog on escape key."""
    #     if event_args.action.keydown and event_args.key.escape:
    #         self.dialog.close()


def content() -> None:
    html_content = plot_csv.readcsv()
    modal_dialog = ModalDialog(html_content)

    with ui.column():
        with ui.row().classes('flex justify-between w-full'):
            # left
            with ui.column().classes('w-2/5'):
                with ui.card().classes('w-full mt-2'):
                    ui.label('Course name and code').classes(
                        'font-serif text-xl font-extrabold pb-8')
                    ui.label('Course Name: Machine Learning').classes(
                        'font-serif text-md pb-4')
                    ui.label('Course Code: FTE35306').classes(
                        'font-serif text-md pb-4')
                # 2
                with ui.card().classes('w-full mt-2'):
                    ui.label('Application description').classes(
                        'font-serif text-xl font-extrabold pb-8')
                    ui.label('The product aims to visualize the predicted crop yields for potatoes in each Dutch province using two different regression models: simple linear regression and K-Nearest Neighbors Regression. The visualization will provide insights into the spatial distribution of predicted yields and help stakeholders in the agrifood chain make informed decisions.').classes(
                        'font-serif text-md pb-4')
                # 3
                with ui.card().classes('w-full mt-2'):
                    ui.label('User description and requirement (PEOPLE)').classes(
                        'font-serif text-xl font-extrabold pb-8')
                    ui.label('Potential users of this product include farmers, agricultural businesses, and policymakers in the agrifood chain. They require a clear and intuitive visualization that shows the predicted crop yields for each province and allows them to compare the performance of the two regression models.').classes(
                        'font-serif text-md pb-4')
                # 4
                with ui.card().classes('w-full mt-2'):
                    ui.label('Purpose of your visualization').classes(
                        'font-serif text-xl font-extrabold pb-8')
                    ui.label('The purpose of this visualization is to present the predicted crop yields for potatoes in each Dutch province using simple linear regression and K-Nearest Neighbors Regression. By visualizing the predicted yields on a map, the stakeholders in the agrifood chain, including farmers, agricultural businesses, and policymakers, can easily identify the spatial patterns and variations in crop yields across different regions. This visualization helps them make informed decisions regarding crop planning, resource allocation, and market analysis.').classes(
                        'font-serif text-md pb-4')
                # # 5
                # with ui.card().classes('w-full mt-2 mb-4'):
                #     ui.label('Dataset description').classes(
                #         'font-serif text-xl font-extrabold pb-8')
                #     ui.label('The dataset used in this project is a spatio-temporal open dataset from the European Commission’s Joint Research Center (JRC). The dataset reports on eleven Dutch provinces for twenty years 1999-2018. It is a raster dataset that contains information on crop yields for different crops, including potatoes.').classes(
                #         'font-serif text-md pb-4')

                # 6
                with ui.card().classes('w-full mt-2 mb-2'):
                    ui.label('Processing methods and tools').classes(
                        'font-serif text-xl font-extrabold pb-8')
                    ui.label('The processing methods used in this project include data cleaning, data preprocessing, and model training. The data cleaning involves removing missing values, checking for outliers, and transforming the data into a format suitable for analysis. The data preprocessing includes feature selection, feature scaling, and data splitting into training and testing sets. The model training involves fitting the two regression models to the training data.').classes(
                        'font-serif text-md pb-4')

            # right
            with ui.column().classes('w-3/5 flex-1 mt-2'):
                ui.label('MAP SHOWING SAMPLE REGIONAL CROP YIELD USING THE NUTS REGIONS').classes(
                    'font-serif font-medium w-full text-center text-lg pb-6')

                ui.html(html_content).classes(
                    'w-full border-solid rounded-lg w-full h-[32rem]')

                # Add a button that opens the dialog when clicked.
                with ui.button('Fullscreen', on_click=modal_dialog.open).classes('mb-2 mx-auto'):
                    ui.icon('launch')

                # 7
                with ui.card().classes('w-full h-[38rem] mt-2 mb-6'):
                    ui.label('Reflection on final results').classes(
                        'font-serif text-xl font-extrabold pb-8')
                    ui.label("The chosen visualization type, a choropleth map, aligns well with the needs of stakeholders in the agrifood chain, including farmers, agricultural businesses, and policymakers. These stakeholders require an intuitive and easily interpretable visualization that allows them to compare and analyze the predicted crop yields across different Dutch provinces. The choropleth map effectively meets this requirement by visually representing the predicted yields through color-coded regions, enabling stakeholders to identify spatial patterns and make informed decisions.").classes('font-serif text-md pb-4')

                    ui.label("The purpose of the product is to present the predicted crop yields for potatoes in each Dutch province using simple linear regression and K-Nearest Neighbors Regression. The chosen visualization type, a choropleth map, successfully aligns with this purpose by visually depicting the predicted yields on a map. This allows stakeholders to easily interpret and compare the crop yields across different regions, aiding in crop planning, resource allocation, and market analysis.").classes('font-serif text-md pb-4')

                    ui.label("The motivation for choosing the choropleth map as the visualization type stems from its ability to provide a comprehensive understanding of the spatial patterns and variations in crop yields. By color-coding the regions based on the predicted yields, the map captures the interplay between the geographic distribution and the predicted values, enabling stakeholders to identify hotspots, regional trends, and potential areas for improvement. The choropleth map offers a visually appealing and intuitive representation that facilitates effective communication and decision-making for stakeholders in the agrifood chain.").classes('font-serif text-md pb-4')

                # with ui.card().classes('w-full mt-8 mb-4'):
                #     ui.label('Extra information').classes(
                #         'font-serif text-xl font-extrabold pb-8')
                #     ui.label('The final result is a choropleth map that shows the predicted crop yields for potatoes in each Dutch province using the two regression models. The map is easy to interpret and provides useful insights into the spatial distribution of predicted yields. However, there are limitations to the analysis, \nas it only considers a limited number of features and does not account for all possible factors that may influence crop yields. Additionally, the accuracy of the predictions may be affected by errors in\n the dataset and the assumptions made in the regression models. Therefore, users should interpret the results with caution and consider other sources of information when making decisions.').classes(
                #         'font-serif text-md pb-4')

        # with ui.button('Next', on_click=lambda: ui.open('/2')).classes('absolute left-[3rem] bottom-[15rem]'):
        #     ui.icon('arrow_forward').classes('text-white')
